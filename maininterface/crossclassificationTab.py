...
class CrossClassification:
	...
	def crossClassification(self, classification, reference, batch = "No", shapefileField = None, rasterOutput = None,  NoDataValue = None):
		...
		try:
			...
			cfg.mx.msgErr26()
		if batch == "No":
			rstrOut = cfg.utls.getSaveFileName(None, cfg.QtWidgetsSCP.QApplication.translate("semiautomaticclassificationplugin", "Save cross classification raster output"), "", "*.tif")
		else:
		...
		if len(rstrOut) > 0:
			if batch == "No":
				...
				try:
					...
					if rD is None:
						...
						l = cfg.iface.addRasterLayer(str(reference), str(cfg.osSCP.path.basename(reference)))
					reml = l
					rD = None
					if cfg.osSCP.path.isfile(classification):
						iClass = cfg.iface.addRasterLayer(str(classification), str(cfg.osSCP.path.basename(classification)))
						remiClass = iClass
					else:
						return "No"
				except:
			...
			if iClass is not None and l is not None:
				...
				if str(crossRstPath).lower().endswith(".tif"):
					pass
				else:
				...
				if o == "No":
					...
					return "No"
				# logger
				cfg.utls.logCondition(str(cfg.inspectSCP.stack()[0][3])+ " " + cfg.utls.lineOfCode(), "cross raster output: " + str(rstrOut))
				# pixel size
				...
				cfg.iface.addRasterLayer(str(crossRstPath), str(cfg.osSCP.path.basename(crossRstPath)))
				rstr = cfg.utls.selectLayerbyName(str(cfg.osSCP.path.basename(crossRstPath)), "Yes")
