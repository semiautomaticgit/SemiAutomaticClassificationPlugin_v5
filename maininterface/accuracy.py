...
class Accuracy:
	...
	def errorMatrix(self, classification, reference, batch = "No", shapefileField = None, rasterOutput = None):
		...
		try:
			...
			cfg.mx.msgErr26()
		if batch == "No":
			rstrOut = cfg.utls.getSaveFileName(None, cfg.QtWidgetsSCP.QApplication.translate("semiautomaticclassificationplugin", "Save error matrix raster output"), "", "*.tif")
		else:
		...
		if len(rstrOut) > 0:
			...
			if iClass is not None and l is not None:
				...
				if str(errorRstPath).lower().endswith(".tif"):
					pass
				else:
				...
				if o == "No":
					...
					return "No"
				# logger
				cfg.utls.logCondition(str(cfg.inspectSCP.stack()[0][3])+ " " + cfg.utls.lineOfCode(), "accuracy raster output: " + str(rstrOut))
				# close GDAL rasters
				...
				cfg.iface.addRasterLayer(str(errorRstPath), str(cfg.osSCP.path.basename(errorRstPath)))
				rstr = cfg.utls.selectLayerbyName(str(cfg.osSCP.path.basename(errorRstPath)), "Yes")
